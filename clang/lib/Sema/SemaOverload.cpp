...
void Sema::HandleFunctionTypeMismatch(PartialDiagnostic &PDiag,
  ...
  if (FromType->isMemberPointerType() && ToType->isMemberPointerType()) {
    ...
    if (!declaresSameEntity(FromMember->getMostRecentCXXRecordDecl(),
      ...
      PDiag << ft_different_class << ToMember->getMostRecentCXXRecordDecl()
...
CompareDerivedToBaseConversions(Sema &S, SourceLocation Loc,
  ...
  if (SCS1.Second == ICK_Pointer_Member && SCS2.Second == ICK_Pointer_Member &&
    ...
    const Type *FromPointeeType1 =
        S.Context.getTypeDeclType(FromMemPointer1->getMostRecentCXXRecordDecl())
            .getTypePtr();
    const Type *ToPointeeType1 =
        S.Context.getTypeDeclType(ToMemPointer1->getMostRecentCXXRecordDecl())
            .getTypePtr();
    const Type *FromPointeeType2 =
        S.Context.getTypeDeclType(FromMemPointer2->getMostRecentCXXRecordDecl())
            .getTypePtr();
    const Type *ToPointeeType2 =
        S.Context.getTypeDeclType(ToMemPointer2->getMostRecentCXXRecordDecl())
            .getTypePtr();
